%!TEX program = xelatex
\expandafter\def\csname CTEX@spaceChar\endcsname{\hspace{1em}}

\documentclass[oneside]{ZJUthesis}

% 该文档中首字符为“%”的均为注释行，不会在论文中出现

% 论文默认为双面模式，需单面模式请将第一行换为如下所示：
% \documentclass[oneside]{ZJUthesis}

% 取消目录中链接的颜色，方便打印
% 如需颜色，请将“false”改为“true”
% \hypersetup{colorlinks=false}

%\usepackage[sectionbib]{chapterbib}

\begin{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 正文字体设定
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\fangsong

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 论文封面部分
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 中文封面内容

% 中图分类号
\classification{TM863}

% 单位代码
\serialnumber{10335}

% 密级，如需密级则将其前“%”去掉
%\SecretLevel{绝密}

% 学号
\PersonalID{3130000569}

\title{基于 SWC 格式的神经重建和可视化系统}
% 如果标题一行写不下，就写成两行，在下面的命令里写第二行，不需要两行则注释掉
% \titletl{第二版}

%英文题目
\Etitle{Neuron Reconstruction and Visualization}
% 如果一行写不下，同中文题目设定，一行写不下则写两行，不需要就注释掉
\Etitletl{ Based on SWC Format }

% 作者
\author{曲衡}

\degree{学士}

% 导师
\supervisor{郑能干}

% 合作导师，如果有的话，去掉注释，
\cpsupervisor{赵挺}

% 专业名称
\major{计算机科学与技术专业}

% 研究方向
% \researchdm{我的方向}

% 所属学院
\institute{计算机科学与技术学院}

%论文提交日期
%\submitdate{2013年9月20日}

% 答辨日期
%\defenddate{2013年9月20日}

% 生成封面
\makeCoverPage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 中文题名页内容
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 论文评阅人信息 注意两字名与三字名，两字职称与三字职称的写法，便于对齐
% 多余的名额直接注释掉即可，比如三个评阅人，把评阅人D，E注释掉即可
\reviewersA{Name\hspace{1.5em}Title\hspace{1.5em}Organization\hspace{1em}}
\reviewersB{Name\hspace{1.5em}Title\hspace{1.5em}Organization\hspace{1em}}
\reviewersC{Name\hspace{1.5em}Title\hspace{1.5em}Organization}
\reviewersD{Name\hspace{1.5em}Title\hspace{1.5em}Organization\hspace{1em}}
\reviewersE{Name\hspace{1.5em}Title\hspace{1.5em}Organization\hspace{1em}}

% 答辩委员会信息，如果某一个单位比较长，
% 请在其它较短后面补上{hspace{Xem}}，X是比最长的单位名少几个字
% 如果实际人数少于6人，多余的注释掉即可
\chairman{Name\hspace{1.5em}Title\hspace{1.5em}Organization}
\commissionerA{Name\hspace{1.5em}Title\hspace{1.5em}Organization\hspace{1em}}
\commissionerB{Name\hspace{1.5em}Title\hspace{1.5em}Organization}
\commissionerC{Name\quad Title\quad Organization}
\commissionerD{Name\hspace{1.5em}Title\hspace{1.5em}Organization\hspace{1em}}
\commissionerE{Name\hspace{1.5em}Title\hspace{1.5em}Organization}

% 生成中文题名页
\maketitle


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 英文封面内容，硕士论文可不要此页
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 英文题名
\englishtitle{Neuron Reconstruction and Visualization}
% 如果题名一行写不下，就写到第二行，不需要则将其注释掉
\englishtitletl{Based on SWC Format}

% 评阅人信息，名字，职称，单位尽量用简写，否则会写不下
\EreviewersA{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}
\EreviewersB{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}
\EreviewersC{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}
\EreviewersD{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}
\EreviewersE{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}

% 答辩委员会信息，同样尽量用简写，否则会写不下
\Echairman{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}
\EcommissionerA{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}
\EcommissionerB{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}
\EcommissionerC{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}
\EcommissionerD{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}
\EcommissionerE{Name\hspace{1.5em}Professional Title\hspace{1.5em}Organization}

% 生成英文封面
\makeenglishtitle


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 原创声明与版权协议页
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\SignautreDateA{2013}{10}{11}
\SignautreDateB{2013}{10}{11}
\SignautreDateC{2013}{10}{11}
% 生成原创声明与版权协议页
\makeOSandCPRTpage


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 论文部分开始
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ZJUfrontmatter

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 勘误页，一般没有
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\include{./Chapters/corrigenda}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 致谢页
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\include{./Chapters/thanks}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 序言页
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\include{./Chapters/preface}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 摘要
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\include{./Chapters/abstractC}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 英文摘要
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\include{./Chapters/abstractE}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 插图列表
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \ZJUListofFigures

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 表格列表
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \ZJUListofTables

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 缩写、符号清单、术语表
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \include{./Chapters/symbol}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 目录页
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ZJUcontents


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 正文内容部分开始
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ZJUmainmatter

\include{./Chapters/chap1}

\include{./Chapters/chap2}

\include{./Chapters/chap3}

\include{./Chapters/chap4}

\include{./Chapters/chap5}


\ZJUbackmatter
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 参考文献
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ZJUthesisbib{thesisbib}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 附录
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\appendix
%\include{./Chapters/appendix}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 索引
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ZJUindex

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 个人简历
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%include{./Chapters/resume}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 发表论文目录
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\include{./Chapters/publications}

\end{document}
